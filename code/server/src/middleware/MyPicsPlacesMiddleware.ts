	...
	public process = async (
		...
		try {
			...
			if (req.method === "PUT" || req.method === "DELETE") {
				...
				const result = await new PlaceRepository().selectOne({
					id: req.body.id,
				});
				// Si un jeu est trouvé et qu’il n’y a pas d’erreur, on le stocke
				if (result && !(result instanceof Error)) {
					place = result as Place;
				}
			}
			...
			if (file) {
				...
				if (req.method === "PUT" && place && place.image) {
					const oldFilePath = `${file.destination}/${place.image}`;
					try {
				...
				if (req.method === "DELETE" && place && place.image) {
					...
					const filePath = `${assetDir}/img/${place.image}`;
					try {
			...
			console.error("Erreur dans placeFileMiddleware:", error);
			res.status(500).json({ error: "Erreur lors du traitement du fichier" });
		}
	};
}

export default MyPicsPlacesMiddleware;
